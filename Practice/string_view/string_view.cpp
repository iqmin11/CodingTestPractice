...
int main()
{
	std::string_view Strview = "asdasdasdasdsa"; //어떤 형태의 문자열 데이터를 받아도, 데이터 그자체를 갖고 있는게 아니라 입력한 곳을 포인터로 가르킬뿐
	...
	//어떤 형태의 문자열을 인자로 받아도 복사생성이 일어나지 않음(동적할당이 일어나지 않는다는 소리)

	//반면 string&를 사용하는 경우
	Str = StrWrong("asdas"); //문자열 리터럴 : 복사생성 일어남(std::string("asdas"))
	Str = StrWrong(cstyle); //c스타일 문자열 : 복사생성 일어남(std::string(cstyle))
