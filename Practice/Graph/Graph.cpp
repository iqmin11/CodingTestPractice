...
struct Node
{
	...
	// 1. 중복 노드가 들어올 일이 없고
	// 2. 오름차순으로 정렬되며
	// 3. 몇개가 들어올 지 모르기 때문에 미리 reserve나 resize를 해야하는 vector보다는 set이 좋다고 생각(힙메모리 생성의 문제)
	// 그러나 중복된 자료가 들어올 일이 없고, 연결되는 노드의 갯수를 미리 알 수 있는 경우,
	// 연결되는 노드의 양이 적을 경우 vector가 훨씬 빠름(메모리 단편화의 문제)
